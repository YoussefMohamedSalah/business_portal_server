...
export const addTask = async (req: Request, res: Response) => {
	...
	if (!task_type) return res.status(404).json({ msg: "Task type is required" });
	...
	try {
		...
		if (task_type === taskType.GROUP_TASK) {
			...
			if (!groupData) return res.status(404).json({ msg: "Project's Group not found" });
		...
		if (!users) return res.status(404).json({ msg: "Company not found" });
...
export const deleteTask = async (req: Request, res: Response) => {
	...
	try {
		...
		if (!task) return res.status(404).json({ msg: "Task not found" });
		await task.remove();
		return res.status(404).json({ msg: "Task deleted" });
	} catch (error) {
...
export const getAllTasksByProjectId = async (req: Request, res: Response) => {
	...
	try {
		...
		if (!group) return res.status(404).json({ msg: "Project's Group not found" });
